...
router.get('/blogs/:id/comments/new', middlewareObj.isLoggedIn, function(req, res){
	...
	blogData.findById(req.params.id, function(err, foundBlog){
		...
		if(err){
			...
			var newCommenter = {id: req.user._id, commenter: req.user.username};
			
			console.log('/blogs/:id/comments/new GET Route is accessed');
			
			console.log('newCommenter at GET: ' + util.inspect(newCommenter, false, null, true /* enable colors */))
			
			res.render('commentsNew.ejs', {blog : foundBlog, commenter: newCommenter});
		}
		...
		if(err){
			...
			blogComment.collection.dropIndexes(function(err, reply){
				...
				if(err){
					...
					var newBlogComment = {text: req.body.blogCommentText};
					...
					blogComment.create(newBlogComment, function(err, newBlogComment){
						...
						if(err){
							...
							newBlogComment.commentedBy = {commenter : req.user.username, id : req.user._id };
...
router.put('/blogs/:id/comments/:comment_id', function(req, res){
	...
	blogComment.findByIdAndUpdate(req.params.comment_id, 
		...
		if(err){
			...
			var query = { '_id': req.params.id, 'comments._id': req.params.comment_id };
			
			var update = {'$set': {'comments.$.text': updatedComment.text}};
			
				
			blogData.findOneAndUpdate(query, update, {new: true}, function(err, foundBlog){
...
router.get('/blogs/:id/comments/:comment_id/edit', middlewareObj.isCommenterAuthorized, function(req, res){
		...
		if(err){
			...
			blogComment.findById(req.params.comment_id, function(err, foundComment){
				...
				if(err){
					...
					res.render('commentsEdit.ejs', {blog: foundBlog, oldComment: foundComment});
...
router.delete('/blogs/:id/comments/:comment_id', middlewareObj.isCommenterAuthorized, function(req, res){
	...
	var query = { '_id': req.params.id, 'comments._id': req.params.comment_id };
	  ...
	  blogData.findById(req.params.id, function(err, foundBlog){
	  	...
	  	if(err){
			...
			blogComment.findByIdAndRemove(req.params.comment_id, function(err, foundComment){
