...
router.post('/blogs', middlewareObj.isLoggedIn, function(req, res){
	...
	var newCreatedBy = {id: req.user._id, bloggername: req.body.username};
	...
	console.log('newBlogData before adding db: ' + util.inspect(newBlogData, false, null, true /* enable colors */))
		...
		if(err){
			...
			blogData.create( newBlogData, function(err, newBlogData){
				...
				if(err){
					...
					console.log('/blogs/new: POST route. New Data is created: ' + newBlogData);
...
router.get('/blogs/:id/edit', middlewareObj.isUserAuthorized, function(req, res){
...
router.put('/blogs/:id', middlewareObj.isUserAuthorized, function(req, res){
	...
	blogData.findByIdAndUpdate(req.params.id, req.body.blog, {new: true}, function(err, updatedBlog){
		...
		if(err){
			...
			console.log('*****' + util.inspect(updatedBlog, false, null, true /* enable colors */));
...
router.delete('/blogs/:id', middlewareObj.isUserAuthorized, function(req, res){
...
router.post('/contact', function(req, res){
	...
	console.log('contactInfo before adding db: ' + util.inspect(contactInfo, false, null, true /* enable colors */))
	
	// contactData.collection.dropIndexes(function(err, reply){
